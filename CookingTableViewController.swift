...
class CookingTableViewController: UITableViewController {
    ...
    fileprivate let concurrentSaveQueue = DispatchQueue(label: "com.gearbox.saveQueue", attributes: .concurrent)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //Fetches the appropriate cooking for the data source layout
        ...
        cell.cookingDetailsLabel.text = details
        if selfWeight != "" && selfWeight != nil {
            cell.cookingWeightLabel.text = selfWeight! + " g"
            cell.cookingWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.cookingWeightLabel.text = manufWeight! + " g"
            cell.cookingWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.cookingWeightLabel.text = "g"
            cell.cookingWeightLabel.textColor = UIColor.black
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddCooking":
            guard let cookingViewController = segue.destination as? CookingViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            cookingViewController.category = filterType
            cookingViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? CookingViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
    ...
    //MARK: Actions
    @IBAction func unwindFromCooking(sender: UIStoryboardSegue){
        return
    }
