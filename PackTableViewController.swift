...
class PackTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        fetchRequest.predicate = NSPredicate(format: "majorCategory == %@", "Packs")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellIdentifier = "PackTableViewCell"
        
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? PackTableViewCell else{
            fatalError("The dequeued cell is not an instance of PackTableViewCell")
        }
        
        //Fetches the appropriate pack for the data source layout
        ...
        cell.packDetailsLabel.text = details
        cell.packCapacityLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1) 
        if selfWeight != "" && selfWeight != nil {
            cell.packWeightLabel.text = selfWeight! + " g"
            cell.packWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.packWeightLabel.text = manufWeight! + " g"
            cell.packWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.packWeightLabel.text = "g"
            cell.packWeightLabel.textColor = UIColor.black
        }
        if capacity != "" && capacity != nil {
            cell.packCapacityLabel.text = capacity! + " L"
        }
        else {
            cell.packCapacityLabel.text = "L"
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddPack":
            guard let packViewController = segue.destination as? PackViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            packViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? PackViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            guard let selectedPackCell = sender as? PackTableViewCell else {
                fatalError("Unexpected sender")
            }
            guard let indexPath = tableView.indexPath(for: selectedPackCell) else {
    ...
    //MARK: Actions
    @IBAction func unwindFromPack(sender: UIStoryboardSegue){
        return
    }
