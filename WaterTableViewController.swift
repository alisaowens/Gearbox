...
class WaterTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        fetchRequest.predicate = NSPredicate(format: "majorCategory == %@", "Water")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //Fetches the appropriate water for the data source layout
        ...
        cell.waterDetailsLabel.text = details
        cell.waterDensityLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        cell.waterCapacityLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        if selfWeight != "" && selfWeight != nil {
            cell.waterWeightLabel.text = selfWeight! + " g"
            cell.waterWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.waterWeightLabel.text = manufWeight! + " g"
            cell.waterWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.waterWeightLabel.text = "g"
            cell.waterWeightLabel.textColor = UIColor.black
        }
        if capacity != "" && capacity != nil {
            cell.waterCapacityLabel.text = capacity! + " L"
        }
        else {
            cell.waterCapacityLabel.text = "L"
        }
        if calculatedDensity != "" && calculatedDensity != nil {
            cell.waterDensityLabel.text = calculatedDensity! + " g/L"
        }
        else {
            cell.waterDensityLabel.text = "g/L"
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddWater":
            guard let waterViewController = segue.destination as? WaterViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            waterViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? WaterViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
    ...
    //MARK: Actions
    @IBAction func unwindFromWater(sender: UIStoryboardSegue){
        return
    }
