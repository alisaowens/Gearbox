...
class CookingViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    
    //MARK: Properties
    @IBOutlet weak var cookingDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var cookingNotes: UITextView!
    ...
    @IBOutlet weak var updatedButton: UIBarButtonItem!
    ...
    override func viewDidLoad() {
        ...
        cookingDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        cookingNotes.delegate=self
        
        self.addDoneButtonOnKeyboard()
        
        let tapRecognizer = UITapGestureRecognizer()
        tapRecognizer.addTarget(self, action: #selector(CookingViewController.didTapView))
        self.view.addGestureRecognizer(tapRecognizer)
        
        //Set up views if editing an existing item.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            cookingDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            cookingNotes.text = gear.value(forKeyPath: "notes") as? String
            
            if let date = gear.value(forKeyPath: "update") as? Date {
                let formatter = DateFormatter()
                formatter.dateFormat = "yyyy-MM-dd"
                let dateString = formatter.string(from: date)
                updatedButton.title = "Updated: " + dateString
            }
        }
        if cookingDetails.text == ""{
            editButton.title = ""
        }
        else{
            saveButton.title = ""
        }
        if cookingNotes.text == ""{
            cookingNotes.text = "Enter notes"}
        if cookingNotes.text == "Enter notes"{
            cookingNotes.textColor = UIColor.lightGray}
        else{cookingNotes.textColor = UIColor.black}
        cookingDetails.becomeFirstResponder()
        switch category{
        case "Stoves":
            if cookingDetails.text == ""{
                navigationItem.title = "New Stove"}
        case "Pots":
            if cookingDetails.text == ""{
                navigationItem.title = "New Pot"}
        case "Fuel":
            if cookingDetails.text == ""{
                navigationItem.title = "New Fuel"}
        case "Other Cooking Gear":
            if cookingDetails.text == ""{
                navigationItem.title = "New Other Cooking Gear"}
        default: fatalError()
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = cookingDetails.text
        let categoryValue = category
        let majorCategory = "Cooking"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let notesValue = cookingNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        cookingNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.cookingNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.cookingDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case cookingDetails?:
            cookingDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            cookingNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(CookingViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(CookingViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
