...
class WaterViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    
    @IBOutlet weak var waterDetails: UITextField!
    ...
    @IBOutlet weak var waterNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        waterDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        capacity.delegate=self
        waterNotes.delegate=self
        
        self.addDoneButtonOnKeyboard()
        
        let tapRecognizer = UITapGestureRecognizer()
        tapRecognizer.addTarget(self, action: #selector(WaterViewController.didTapView))
        self.view.addGestureRecognizer(tapRecognizer)
        
        //Set up views if editing an existing Water.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            waterDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            waterNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if waterNotes.text == ""{
            waterNotes.text = "Enter notes"}
        if waterNotes.text == "Enter notes"{
            waterNotes.textColor = UIColor.lightGray}
        else{waterNotes.textColor = UIColor.black}
        waterDetails.becomeFirstResponder()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        updateDensity()
        let detailsValue = waterDetails.text
        let majorCategory = "Water"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let capacityValue = capacity.text
        let densityValue = density.text
        let notesValue = waterNotes.text
        if isEdit == false {
        ...
        if let gear = gear {
            ...
            gear.setValue(capacityValue, forKeyPath: "size")
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        waterNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.waterNotes.inputAccessoryView = notesToolbar
        ...
        self.waterDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case waterDetails?:
            waterDetails.resignFirstResponder()
        ...
        case capacity?:
            ...
            waterNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(WaterViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(WaterViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
    ...
    func updateDensity() {
        ...
        if capacity.text != "" {
            guard let _capacity = Double(capacity.text!) else {
                return
            }
        ...
        density.text = String(format: "%.1f", weight!/capacityValue!)
    }
}
