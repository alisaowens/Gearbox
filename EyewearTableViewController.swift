...
class EyewearTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        fetchRequest.predicate = NSPredicate(format: "majorCategory == %@", "Eyewear")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellIdentifier = "EyewearTableViewCell"
        
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? EyewearTableViewCell else{
            fatalError("The dequeued cell is not an instance of EyewearTableViewCell")
        }
        
        //Fetches the appropriate eyewear for the data source layout
        ...
        cell.eyewearDetailsLabel.text = details
        if selfWeight != "" && selfWeight != nil {
            cell.eyewearWeightLabel.text = selfWeight! + " g"
            cell.eyewearWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.eyewearWeightLabel.text = manufWeight! + " g"
            cell.eyewearWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.eyewearWeightLabel.text = "g"
            cell.eyewearWeightLabel.textColor = UIColor.black
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddEyewear":
            guard let eyewearViewController = segue.destination as? EyewearViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            eyewearViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? EyewearViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            guard let selectedEyewearCell = sender as? EyewearTableViewCell else {
                fatalError("Unexpected sender")
            }
            guard let indexPath = tableView.indexPath(for: selectedEyewearCell) else {
    ...
    //MARK: Actions
    @IBAction func unwindFromEyewear(sender: UIStoryboardSegue){
        return
    }
