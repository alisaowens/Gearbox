...
class LightingTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        fetchRequest.predicate = NSPredicate(format: "majorCategory == %@", "Lighting")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellIdentifier = "LightingTableViewCell"
        
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? LightingTableViewCell else{
            fatalError("The dequeued cell is not an instance of LightingTableViewCell")
        }
        
        //Fetches the appropriate lighting for the data source layout
        ...
        let capacity = gearItem.value(forKeyPath: "size") as? String
        cell.lightingDetailsLabel.text = details
        if selfWeight != "" && selfWeight != nil {
            cell.lightingWeightLabel.text = selfWeight! + " g"
            cell.lightingWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.lightingWeightLabel.text = manufWeight! + " g"
            cell.lightingWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.lightingWeightLabel.text = "g"
            cell.lightingWeightLabel.textColor = UIColor.black
        }
        if capacity != "" && capacity != nil {
            cell.lightingLumensLabel.text = capacity! + " lumens"
        }
        else {
            cell.lightingLumensLabel.text = "lumens"
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddLighting":
            guard let lightingViewController = segue.destination as? LightingViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            lightingViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? LightingViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            guard let selectedLightingCell = sender as? LightingTableViewCell else {
                fatalError("Unexpected sender")
            }
            guard let indexPath = tableView.indexPath(for: selectedLightingCell) else {
    ...
    //MARK: Actions
    @IBAction func unwindFromLighting(sender: UIStoryboardSegue){
        return
    }
