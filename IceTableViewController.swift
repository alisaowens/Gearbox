...
class IceTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cellIdentifier = "IceTableViewCell"
        
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? IceTableViewCell else{
            fatalError("The dequeued cell is not an instance of IceTableViewCell")
        }
        
        //Fetches the appropriate ice for the data source layout
        ...
        cell.iceDetailsLabel.text = details
        if selfWeight != "" && selfWeight != nil {
            cell.iceWeightLabel.text = selfWeight! + " g"
            cell.iceWeightLabel.textColor = UIColor.black
        }
        else if manufWeight != "" && manufWeight != nil {
            cell.iceWeightLabel.text = manufWeight! + " g"
            cell.iceWeightLabel.textColor = UIColor(red: 0.498, green: 0, blue: 0, alpha: 1)
        }
        else {
            cell.iceWeightLabel.text = "g"
            cell.iceWeightLabel.textColor = UIColor.black
        }
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "AddIce":
            guard let iceViewController = segue.destination as? IceViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            iceViewController.category = filterType
            iceViewController.isEdit = false
        case "ShowDetail":
            guard let detailViewController = segue.destination as? IceViewController else{
                fatalError("Unexpected destination: \(segue.destination)")
            }
            guard let selectedIceCell = sender as? IceTableViewCell else {
                fatalError("Unexpected sender")
            }
            guard let indexPath = tableView.indexPath(for: selectedIceCell) else {
    ...
    //MARK: Actions
    @IBAction func unwindFromIce(sender: UIStoryboardSegue){
        return
    }
