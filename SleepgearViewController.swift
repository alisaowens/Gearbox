...
class SleepgearViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    @IBOutlet weak var sleepgearDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var sleepgearNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        sleepgearDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        sleepgearNotes.delegate=self
        ...
        if let gear = gear {
            ...
            sleepgearDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            sleepgearNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if sleepgearDetails.text == ""{
            editButton.title = ""
        }
        else{
            saveButton.title = ""
        }
        if sleepgearNotes.text == ""{
            sleepgearNotes.text = "Enter notes"}
        if sleepgearNotes.text == "Enter notes"{
            sleepgearNotes.textColor = UIColor.lightGray}
        else{sleepgearNotes.textColor = UIColor.black}
        sleepgearDetails.becomeFirstResponder()
        switch category {
        case "Tents":
            if sleepgearDetails.text == ""{
                navigationItem.title = "New Tent"}
        case "Bivy Sacks & Tarps":
            if sleepgearDetails.text == ""{
                navigationItem.title = "New Bivy Sack/Tarp"}
        case "Sleeping Bags":
            if sleepgearDetails.text == ""{
                navigationItem.title = "New Sleeping Bag"}
        case "Sleeping Pads":
            if sleepgearDetails.text == ""{
                navigationItem.title = "New Sleeping Pad"}
        case "Other Sleeping Gear":
            if sleepgearDetails.text == ""{
                navigationItem.title = "New Other Sleeping Gear"}
        default: fatalError()
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = sleepgearDetails.text
        let categoryValue = category
        let majorCategory = "Sleepgear"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let notesValue = sleepgearNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        sleepgearNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        let done: UIBarButtonItem  = UIBarButtonItem(title: "Next", style: UIBarButtonItemStyle.done, target: self, action: #selector(SleepgearViewController.doneButtonAction))
        ...
        self.sleepgearNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.sleepgearDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case sleepgearDetails?:
            sleepgearDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            sleepgearNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(SleepgearViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(SleepgearViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
