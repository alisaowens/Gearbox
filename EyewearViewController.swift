...
class EyewearViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    
    @IBOutlet weak var eyewearDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var eyewearNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        eyewearDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        eyewearNotes.delegate=self
        ...
        //Set up views if editing an existing Eyewear.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            eyewearDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            eyewearNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if eyewearDetails.text == ""{
            editButton.title = ""
        }
        else{
            saveButton.title = ""
        }
        if eyewearNotes.text == ""{
            eyewearNotes.text = "Enter notes"}
        if eyewearNotes.text == "Enter notes"{
            eyewearNotes.textColor = UIColor.lightGray}
        else{eyewearNotes.textColor = UIColor.black}
        eyewearDetails.becomeFirstResponder()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = eyewearDetails.text
        let majorCategory = "Eyewear"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let notesValue = eyewearNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        eyewearNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        let done: UIBarButtonItem  = UIBarButtonItem(title: "Next", style: UIBarButtonItemStyle.done, target: self, action: #selector(EyewearViewController.doneButtonAction))
        ...
        self.eyewearNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.eyewearDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case eyewearDetails?:
            eyewearDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            eyewearNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(EyewearViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(EyewearViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
