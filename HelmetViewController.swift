...
class HelmetViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    
    @IBOutlet weak var helmetDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var helmetNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        helmetDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        helmetNotes.delegate=self
        ...
        //Set up views if editing an existing Helmet.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            helmetDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            helmetNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if helmetNotes.text == ""{
            helmetNotes.text = "Enter notes"}
        if helmetNotes.text == "Enter notes"{
            helmetNotes.textColor = UIColor.lightGray}
        else{helmetNotes.textColor = UIColor.black}
        helmetDetails.becomeFirstResponder()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = helmetDetails.text
        let majorCategory = "Helmets"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let notesValue = helmetNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        helmetNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.helmetNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.helmetDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case helmetDetails?:
            helmetDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            helmetNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(HelmetViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(HelmetViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
