...
class LightingViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    
    @IBOutlet weak var lightingDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var lumens: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var lightingNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        lightingDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        lumens.delegate=self
        lightingNotes.delegate=self
        
        self.addDoneButtonOnKeyboard()
        
        let tapRecognizer = UITapGestureRecognizer()
        tapRecognizer.addTarget(self, action: #selector(LightingViewController.didTapView))
        self.view.addGestureRecognizer(tapRecognizer)
        
        //Set up views if editing an existing Lighting.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            lightingDetails.text = gear.value(forKeyPath: "details") as? String
            selfWeight.text = gear.value(forKeyPath: "selfWeight") as? String
            manufWeight.text = gear.value(forKeyPath: "manufWeight") as? String
            lumens.text  = gear.value(forKeyPath: "size") as? String
            if gear.value(forKeyPath: "photo") != nil {
            ...
            lightingNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if lightingDetails.text == ""{
            editButton.title = ""
        }
        else{
            saveButton.title = ""
        }
        if lightingNotes.text == ""{
            lightingNotes.text = "Enter notes"}
        if lightingNotes.text == "Enter notes"{
            lightingNotes.textColor = UIColor.lightGray}
        else{lightingNotes.textColor = UIColor.black}
        lightingDetails.becomeFirstResponder()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = lightingDetails.text
        let majorCategory = "Lighting"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let sizeValue = lumens.text
        let notesValue = lightingNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        lightingNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.lightingNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.lumens.inputAccessoryView = doneToolbar
        self.lightingDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case lightingDetails?:
            lightingDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            lumens.becomeFirstResponder()
        case lumens?:
            lumens.resignFirstResponder()
            lightingNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(LightingViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(LightingViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
