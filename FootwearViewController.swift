...
class FootwearViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    @IBOutlet weak var footwearDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var size: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var footwearNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        footwearDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        size.delegate=self
        footwearNotes.delegate=self
        
        self.addDoneButtonOnKeyboard()
        
        let tapRecognizer = UITapGestureRecognizer()
        tapRecognizer.addTarget(self, action: #selector(FootwearViewController.didTapView))
        self.view.addGestureRecognizer(tapRecognizer)
        
        //Set up views if editing an existing Footwear.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            footwearDetails.text = gear.value(forKeyPath: "details") as? String
            selfWeight.text = gear.value(forKeyPath: "selfWeight") as? String
            manufWeight.text = gear.value(forKeyPath: "manufWeight") as? String
            size.text = gear.value(forKeyPath: "size") as? String
            if gear.value(forKeyPath: "photo") != nil {
            ...
            footwearNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if footwearDetails.text == ""{
            editButton.title = ""
        }
        else{
            saveButton.title = ""
        }
        if footwearNotes.text == ""{
            footwearNotes.text = "Enter notes"}
        if footwearNotes.text == "Enter notes"{
            footwearNotes.textColor = UIColor.lightGray}
        else{footwearNotes.textColor = UIColor.black}
        footwearDetails.becomeFirstResponder()
        switch category {
        case "Ski Boots":
            if footwearDetails.text == ""{
                navigationItem.title = "New Ski Boots"}
        case "Mountain Boots":
            if footwearDetails.text == ""{
                navigationItem.title = "New Mountain Boots"}
        case "Trail Shoes":
            if footwearDetails.text == ""{
                navigationItem.title = "New Trail Shoes"}
        case "Rock Shoes":
            if footwearDetails.text == ""{
                navigationItem.title = "New Rock Shoes"}
        case "Other Footwear":
            if footwearDetails.text == ""{
                navigationItem.title = "New Other Footwear"}
        default: fatalError()
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = footwearDetails.text
        let categoryValue = category
        let majorCategory = "Footwear"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let sizeValue = size.text
        let notesValue = footwearNotes.text
        if isEdit == false {
    ...
    @IBAction func selectImageFromPhotoLibrary(_ sender: UITapGestureRecognizer) {
        footwearNotes.resignFirstResponder()
        if editButton.title != "Edit"{
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.footwearNotes.inputAccessoryView = notesToolbar
        
        self.size.inputAccessoryView = doneToolbar
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.footwearDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case footwearDetails?:
            footwearDetails.resignFirstResponder()
        ...
        case manufWeight?:
            ...
            footwearNotes.becomeFirstResponder()
        default:
            activeField?.resignFirstResponder()
        }}
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(FootwearViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(FootwearViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
