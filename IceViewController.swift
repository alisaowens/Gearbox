...
class IceViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate {
    //MARK: Properties
    
    @IBOutlet weak var iceDetails: UITextField!
    @IBOutlet weak var selfWeight: UITextField!
    @IBOutlet weak var manufWeight: UITextField!
    @IBOutlet weak var photoImageView: UIImageView!
    @IBOutlet weak var iceNotes: UITextView!
    ...
    override func viewDidLoad() {
        ...
        iceDetails.delegate=self
        selfWeight.delegate=self
        manufWeight.delegate=self
        iceNotes.delegate=self
        
        self.addDoneButtonOnKeyboard()
        
        let tapRecognizer = UITapGestureRecognizer()
        tapRecognizer.addTarget(self, action: #selector(IceViewController.didTapView))
        self.view.addGestureRecognizer(tapRecognizer)
        
        //Set up views if editing an existing item.
        if let gear = gear {
            navigationItem.title = gear.value(forKeyPath: "details") as? String
            iceDetails.text = gear.value(forKeyPath: "details") as? String
            ...
            iceNotes.text = gear.value(forKeyPath: "notes") as? String
        ...
        if iceNotes.text == ""{
            iceNotes.text = "Enter notes"}
        if iceNotes.text == "Enter notes"{
            iceNotes.textColor = UIColor.lightGray}
        else{iceNotes.textColor = UIColor.black}
        iceDetails.becomeFirstResponder()
        switch category{
        case "Ice Axes":
            if iceDetails.text == ""{
                navigationItem.title = "New Ice Axe"}
        case "Crampons":
            if iceDetails.text == ""{
                navigationItem.title = "New Crampons"}
        case "Leashes":
            if iceDetails.text == ""{
                navigationItem.title = "New Leash"}
        case "Other Ice Gear":
            if iceDetails.text == ""{
                navigationItem.title = "New Other Ice Gear"}
        default: fatalError()
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        let detailsValue = iceDetails.text
        let categoryValue = category
        let majorCategory = "Ice"
        let selfWeightValue = selfWeight.text
        let manufWeightValue = manufWeight.text
        let notesValue = iceNotes.text
        if isEdit == false {
    ...
    func addDoneButtonOnKeyboard() {
        ...
        self.iceNotes.inputAccessoryView = notesToolbar
        
        self.selfWeight.inputAccessoryView = doneToolbar
        self.manufWeight.inputAccessoryView = doneToolbar
        self.iceDetails.inputAccessoryView = doneToolbar
    }
    
    @objc func doneButtonAction() {
        switch activeField{
        case iceDetails?:
            iceDetails.resignFirstResponder()
    ...
    fileprivate func registerKeyboardNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(IceViewController.keyboardWillShow), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(IceViewController.keyboardWillHide), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
